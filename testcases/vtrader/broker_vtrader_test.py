...
class MarketOrderTestCase(MockVtraderServerTestCase):
    def testBuyAndSell(self):
        ...
        self.assertEqual(order.getExecutionInfo().getCommission(), VtraderBroker.COMMISSION_PER_TRADE)
        self.assertEqual(len(vtrader.getActiveOrders()), 0)
        self.assertEqual(vtrader.getCash(), 100 - 10 - VtraderBroker.COMMISSION_PER_TRADE)
        ...
        self.assertEqual(order.getExecutionInfo().getCommission(), VtraderBroker.COMMISSION_PER_TRADE)
        self.assertEqual(len(vtrader.getActiveOrders()), 0)
        self.assertEqual(vtrader.getCash(), 100 - 2 * VtraderBroker.COMMISSION_PER_TRADE)
    ...
    def testSellShort(self):
        vtrader, backtest = self.get_brokers(200, barFeed=barfeed.BaseBarFeed(bar.Frequency.MINUTE))
        barsBuilder = BarsBuilder(MockVtraderServerTestCase.TestInstrument, bar.Frequency.MINUTE)
        ...
        self.assertEqual(order.getExecutionInfo().getCommission(),  VtraderBroker.COMMISSION_PER_TRADE)
        self.assertEqual(vtrader.getShares(MockVtraderServerTestCase.TestInstrument), 1)
        self.assertEqual(vtrader.getCash(), 100 - VtraderBroker.COMMISSION_PER_TRADE)
        ...
        self.assertEqual(order.getExecutionInfo().getCommission(), VtraderBroker.COMMISSION_PER_TRADE)
        self.assertEqual(vtrader.getShares(MockVtraderServerTestCase.TestInstrument), -1)
        self.assertEqual(vtrader.getCash(), 300 - 2 * VtraderBroker.COMMISSION_PER_TRADE)
        ...
        self.assertEqual(order.getExecutionInfo().getCommission(), VtraderBroker.COMMISSION_PER_TRADE)
        self.assertEqual(vtrader.getShares(MockVtraderServerTestCase.TestInstrument), 0)
        self.assertEqual(vtrader.getCash(), 200 - 3 * VtraderBroker.COMMISSION_PER_TRADE)
...
class LimitOrderTestCase(MockVtraderServerTestCase):
    def testBuyAndSell_HitTargetPrice(self):
        ...
        self.assertEqual(order.getExecutionInfo().getCommission(), VtraderBroker.COMMISSION_PER_TRADE)
        self.assertEqual(len(vtrader.getActiveOrders()), 0)
        self.assertEqual(vtrader.getCash(), 100 - 10 + 15 - 2 * VtraderBroker.COMMISSION_PER_TRADE)
        self.assertEqual(vtrader.getShares(MockVtraderServerTestCase.TestInstrument), 0)
        self.assertEqual(cb.eventCount, 2)
